//　統計ver2
/* ver1との違いは要素数とデータを乱数を用いて生成することと、配列を要素数分のメモリを動的に確保すること。*/
...
void hyouji(double* p_array, int n, double heikinn);	//表示を行います
void deviation(double* p_array, int n);					//平均、分散、標準偏差を算出します
void seiretu(double* p_array, int n);					//昇順に並べ替え、最小値、最大値、中央値を算出します
void mode(double* p_array, int n);						//最頻値を算出します
...
switch(p_array[i] / 10)　だと、うまくいかない
↓
int v = p_array[i] / 10;
switch(v)	
のようにswitchの中で評価しないで、変数で値を出してからswitchで評価
